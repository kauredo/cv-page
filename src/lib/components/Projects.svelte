...
<section aria-labelledby="projects-heading">
	<div class="mb-8 flex items-end justify-between">
		...
		<a
			href="/projects"
			class="group flex items-center gap-1.5 text-sm font-medium text-vasco-600 hover:text-vasco-700 dark:text-vasco-400 dark:hover:text-vasco-300"
		>
			See all
			<svg class="h-4 w-4 transition-transform group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
	...
	{#if featuredProject}
		<a
			...
			class="group relative mb-8 block overflow-hidden rounded-xl border border-slate-200 bg-white transition-all duration-300 hover:border-hoop-300 hover:shadow-lg hover:shadow-hoop-500/10 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-hoop-500/50 dark:hover:shadow-hoop-500/20"
			data-sveltekit-preload-data="hover"
		>
			<div class="grid gap-0 lg:grid-cols-2">
				<!-- Image side -->
				<div class="relative aspect-video overflow-hidden bg-slate-100 lg:aspect-auto lg:h-full dark:bg-slate-800">
					...
					{/if}
					<!-- Featured badge -->
					<div class="absolute top-4 left-4 rounded-full bg-hoop-500 px-3 py-1 text-xs font-semibold text-white shadow-lg">
						Featured
					</div>
				</div>
				...
				<div class="flex flex-col justify-center p-6 lg:p-8">
					...
					{/if}
					<div class="mt-6">
						<span class="inline-flex items-center gap-2 text-sm font-semibold text-hoop-600 dark:text-hoop-400">
							View project
