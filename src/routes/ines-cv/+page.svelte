...
<section class="screen-only-nav">
	<nav>
		<div class="flex items-center gap-2">
			<span class="text-sm text-gray-500 dark:text-gray-400">{basics.name}</span>
		</div>
...
<style>
	/* Ines CV color theme - yellow/gold instead of blue */
	.ines-cv {
		--color-vasco-100: #fffde7;
		--color-vasco-200: #fff9c4;
		--color-vasco-300: #fff59d;
		--color-vasco-400: #fff176;
		--color-vasco-500: #ffee58;
		--color-vasco-600: #ffeb3b;
		--color-vasco-700: #fdd835;
		--color-vasco-800: #fbc02d;
		--color-vasco-900: #f9a825;
		border-left: 4px solid var(--color-vasco-700);
		padding-left: 1rem;
	}

	.ines-cv :global(header) {
		border-bottom: none;
		padding-bottom: 0;
	}

	.ines-cv :global(.avatar) {
		width: 120px;
		height: 120px;
		border-radius: 15%;
		border: 2px solid var(--color-vasco-200);
	}

	.ines-cv :global(h2) {
		border-bottom: 2px solid var(--color-vasco-700);
	}

	.ines-cv :global(.company) {
		color: var(--color-vasco-700);
	}

	.ines-cv :global(.job-title-group span::before) {
		color: var(--color-vasco-800);
	}

	.ines-cv :global(.skill-item) {
		border-bottom: 1px dotted var(--color-vasco-700);
	}

	.ines-cv :global(.contact-item a) {
		color: var(--color-vasco-800);
	}

	.ines-cv :global(.contact-item a:hover) {
		text-decoration: underline;
	}

	/* References section */
	.ines-cv :global(.references) {
		margin-top: 0.5rem;
		font-size: 11px;
	}

	.ines-cv :global(.references h2) {
		font-size: 14px;
		margin: 0 0 0.4rem;
		padding: 0 0 0.25rem;
		border-bottom: 1px solid var(--color-vasco-700);
	}

	.ines-cv :global(.references-list) {
		display: flex;
		flex-direction: column;
		gap: 2px;
	}

	.ines-cv :global(.reference-item) {
		line-height: 1.2;
	}

	.ines-cv :global(.reference-item .ref-contact),
	.ines-cv :global(.reference-item .ref-rel) {
		color: #555;
		font-weight: 400;
		font-size: 10px;
	}
	...
	@media print {
		...
		.ines-cv :global(a) {
			color: var(--color-vasco-900) !important;
		}

		/* Override global styles - allow page breaks between sections */
		.ines-cv :global(section) {
		...
		.ines-cv :global(.experience) {
			overflow: hidden;
		}

		.ines-cv :global(.education) {
			break-inside: avoid;
		}

		.ines-cv :global(.experience .absolute) {
			height: 180vh !important;
		}

		/* Keep individual items from breaking */
		.ines-cv :global(.experience > div > div),
		.ines-cv :global(.education > div > div) {
